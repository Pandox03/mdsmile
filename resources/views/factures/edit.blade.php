...
<div class="space-y-8">
    <div class="rounded-xl border border-[#967A4B]/40 bg-zinc-900/80 px-5 py-4">
        <h1 class="text-xl font-bold text-[#967A4B]">Modifier facture {{ $facture->numero }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('factures.update', $facture) }}" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <select name="doc_id" required class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                        @foreach($docs as $d)
                            <option value="{{ $d->id }}" {{ old('doc_id', $facture->doc_id) == $d->id ? 'selected' : '' }}>{{ $d->display_name ?? $d->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Date facture <span class="text-red-400">*</span></label>
                    <input type="date" name="date_facture" value="{{ old('date_facture', $facture->date_facture->format('Y-m-d')) }}" required class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Montant (DHS) <span class="text-red-400">*</span></label>
                    <input type="number" name="montant" value="{{ old('montant', $facture->montant_comptabilise_affiche) }}" min="0.01" step="0.01" required class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-right text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('factures.show', $facture) }}" class="rounded-lg border border-zinc-600 px-6 py-2.5 text-sm font-medium text-zinc-400 hover:bg-zinc-800">
                Annuler
            </a>
        </div>
    </form>
