...
<div class="space-y-8">
    ...
    <div class="rounded-xl border border-[#967A4B]/40 bg-zinc-900/80 px-5 py-4">
        ...
        <p class="mt-1 text-sm text-zinc-400">Filtrer par mois/année. Le report intègre les travaux passés et les encaissements enregistrés par mois. Les encaissements ne peuvent être ajoutés ou retirés que pour le <strong class="text-zinc-300">mois calendaire en cours</strong>.</p>
    ...
    <form method="GET" action="{{ route('doc.situations.index') }}" class="rounded-xl border border-[#967A4B]/20 bg-zinc-900/80 p-4">
        <div class="flex flex-wrap items-end gap-4">
            ...
            <div>
                <label class="mb-1 block text-xs font-medium text-zinc-500">Mois</label>
                <select name="month" class="auth-input rounded-lg border border-zinc-600 bg-zinc-800/90 px-3 py-2 text-sm text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                    @foreach($monthOptions as $mNum => $mLabel)
                        <option value="{{ $mNum }}" {{ (int)($month ?? 0) === (int)$mNum ? 'selected' : '' }}>{{ $mLabel }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="mb-1 block text-xs font-medium text-zinc-500">Année</label>
                <input type="number" name="year" min="2000" max="2100" value="{{ $year ?? now()->year }}" class="auth-input w-28 rounded-lg border border-zinc-600 bg-zinc-800/90 px-3 py-2 text-sm text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
            </div>
    ...
    <div class="rounded-xl border border-[#967A4B]/20 bg-zinc-900/80 overflow-hidden">
        <div class="border-b border-[#967A4B]/30 bg-zinc-800/80 px-4 py-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <a href="{{ route('doc.situations.pdf', ['doc_id' => $doc->id, 'month' => $month, 'year' => $year]) }}" target="_blank" class="rounded-lg border border-[#967A4B] bg-[#967A4B] px-4 py-2.5 text-sm font-medium text-black hover:bg-[#B8986B]">
                Télécharger PDF
            </a>
        </div>
        <div class="grid gap-3 border-b border-zinc-800 bg-zinc-900/70 p-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="rounded-lg border border-zinc-700 bg-zinc-900/80 p-3">
                <p class="text-xs text-zinc-500">Travaux du mois</p>
                <p class="mt-1 text-lg font-semibold text-zinc-200">{{ number_format($travauxMonthTotal, 0, ',', ' ') }} DHS</p>
            </div>
            <div class="rounded-lg border border-zinc-700 bg-zinc-900/80 p-3">
                <p class="text-xs text-zinc-500">Total encaissé (situation, ce mois)</p>
                <p class="mt-1 text-lg font-semibold text-zinc-200">{{ number_format($montantRecuMois, 0, ',', ' ') }} DHS</p>
                ...
                @enderror
                @else
                <p class="mt-2 text-xs text-zinc-500">Mois clos — saisie et suppression des encaissements désactivées.</p>
                @endif
            </div>
            <div class="rounded-lg border border-[#967A4B]/40 bg-[#967A4B]/10 p-3">
                <p class="text-xs text-zinc-500">Reste à payer (fin du mois)</p>
                <p class="mt-1 text-lg font-bold text-[#967A4B]">{{ number_format($soldeFinMois, 0, ',', ' ') }} DHS</p>
            </div>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full min-w-[700px] text-left text-sm">
                ...
                <tbody>
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-4 py-8 text-center text-zinc-500">Aucun travail sur ce mois.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="border-t border-zinc-800 bg-zinc-900/60 px-4 py-4">
            ...
            <div class="mt-3 overflow-x-auto">
                <table class="w-full min-w-[420px] text-left text-sm">
                    ...
                    <tbody>
                        @forelse($encaissementsDuMois as $enc)
                        ...
                        @empty
                        <tr>
                            <td colspan="{{ ($situationEncaissementEditable ?? false) ? 3 : 2 }}" class="px-4 py-6 text-center text-zinc-500">Aucun encaissement enregistré pour ce mois.</td>
                        </tr>
                        @endforelse
                    </tbody>
                    @if($encaissementsDuMois->isNotEmpty())
                    <tfoot>
                        <tr class="border-t border-zinc-700 bg-zinc-800/30">
                            ...
                            <td class="px-4 py-2.5 text-right text-sm font-semibold text-zinc-200">{{ number_format((float) $encaissementsDuMois->sum('montant'), 0, ',', ' ') }}</td>
                            @if($situationEncaissementEditable ?? false)
                            <td></td>
                            @endif
                        </tr>
        ...
        <div class="border-t border-[#967A4B]/30 bg-zinc-800/50 px-4 py-3 flex justify-end">
            <p class="text-lg font-bold text-[#967A4B]">Situation du mois : {{ number_format($situationTotal, 0, ',', ' ') }} DHS</p>
        </div>
        @endif
    </div>
    @endif
