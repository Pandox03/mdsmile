...
<div class="space-y-8" x-data="travailForm()">
    <div class="rounded-xl border border-[#967A4B]/40 bg-zinc-900/80 px-5 py-4">
        <h1 class="text-xl font-bold text-[#967A4B]">Modifier le travail</h1>
    </div>
    ...
    <form method="POST" action="{{ route('travaux.update', $travail) }}" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div x-show="addNewDoc != 1" x-cloak>
                    <select name="doc_id" class="auth-input w-full max-w-md rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                        ...
                        @foreach($docs as $doc)
                            <option value="{{ $doc->id }}" {{ old('doc_id', $travail->doc_id) == $doc->id ? 'selected' : '' }}>{{ $doc->display_name }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="rounded-xl border border-[#967A4B]/20 bg-zinc-900/80 p-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                    ...
                    <input type="text" name="patient" value="{{ old('patient', $travail->patient) }}" required class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Numéro de fiche</label>
                    <input type="text" name="numero_fiche" value="{{ old('numero_fiche', $travail->numero_fiche) }}" placeholder="—" class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 placeholder-zinc-500 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Âge du patient</label>
                    <input type="number" name="patient_age" value="{{ old('patient_age', $travail->patient_age) }}" min="1" max="150" placeholder="—" class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 placeholder-zinc-500 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Date d'entrée</label>
                    <input type="date" name="date_entree" value="{{ old('date_entree', $travail->date_entree->format('Y-m-d')) }}" required class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Date de livraison</label>
                    <input type="date" name="date_livraison" value="{{ old('date_livraison', $travail->date_livraison->format('Y-m-d')) }}" required class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Date d'essiage</label>
                    <input type="date" name="date_essiage" value="{{ old('date_essiage', $travail->date_essiage?->format('Y-m-d')) }}" class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
                @hasanyrole('manager|secretaire')
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Prix total (DHS)</label>
                    <input type="number" id="prix-dhs-input" name="prix_dhs" value="{{ old('prix_dhs', $travail->prix_dhs) }}" min="0" step="0.01" class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                </div>
                @endhasanyrole
                <div>
                    <label class="mb-1 block text-sm font-medium text-zinc-400">Statut</label>
                    <select name="statut" class="auth-input w-full rounded-lg border border-zinc-600 bg-zinc-800/90 px-4 py-2.5 text-zinc-100 focus:border-[#967A4B] focus:outline-none focus:ring-1 focus:ring-[#967A4B]/30">
                        @foreach($statutLabels as $value => $label)
                            <option value="{{ $value }}" {{ old('statut', $travail->statut) === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        {{-- Teeth + prestations (React odontogram) --}}
        <div class="rounded-xl border border-[#967A4B]/20 bg-zinc-900/80 p-6">
            <h2 class="mb-4 text-lg font-semibold text-[#967A4B]">Dents concernées</h2>
            <p class="mb-4 text-sm text-zinc-500">Cliquez sur les dents, choisissez une prestation puis appliquez. Vous pouvez appliquer des prestations différentes à différents groupes de dents.</p>

            @php
                $initialTeethForJs = $travail->teeth->map(fn ($t) => [
                    'tooth_number' => $t->tooth_number,
                    'stock_id' => $t->stock_id,
                    'stock_name' => $t->stock?->name ?? null,
                    'prestation_id' => $t->prestation_id,
                    'prestation_name' => $t->prestation?->name ?? 'Dent',
                ])->values()->all();
            ...
            @endphp
            <script>
                window.MDSMILE_STOCK_ITEMS = @json($stockItems ?? []);
                window.MDSMILE_PRESTATIONS = @json($prestationsForJs);
                window.MDSMILE_INITIAL_TEETH = @json($initialTeethForJs);
...
(function() {
    ...
    function updatePrix() {
        ...
        var total = 0;
        teethList.forEach(function(t) { if (t.prestation_id != null && t.prestation_id !== '') total += getUnitPrice(docId, t.prestation_id); });
        prixEl.value = total;
    }
